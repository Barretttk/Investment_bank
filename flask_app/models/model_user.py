...
# ===================  GET ONE EMAIL ALSO VALIDATE=======================
    ...
    @classmethod
    def get_one_by_username(cls, data:dict) ->None:
...
#=======================  LOGIN VALIDATE  ======================
    ...
    def validate_login(data:dict) -> bool:
        ...
        elif not EMAIL_REGEX.match(data['username']):
            flash("Invalid username address!", 'err_user_email_login')
            is_valid = False
        else:
            #cheeck db to see if username already exist
            existing_user = User.get_one_by_username({'username' : data['username']})
            if not existing_user:
                flash("Email not found!", 'err_user_username_login')
                is_valid = False
